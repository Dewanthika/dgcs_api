...
export class User extends Document {
  ...
  @Prop({ required: true }) fName: string;
  @Prop({ required: true }) lName: string;
  @Prop() DOB: Date;
  @Prop({ type: AddressSchema }) address: Address;
  @Prop() phone: string;
  @Prop({ required: true }) password: string;
  @Prop({ type: Types.ObjectId, required: true, ref: Company.name }) companyID:
    | CreateCompanyDto
    | Types.ObjectId;
  @Prop({ type: Types.ObjectId, required: true, ref: UserType.name })
  userTypeID: CreateUserTypeDto | Types.ObjectId;
  @Prop({ default: Date.now }) joinedDate: Date;
}
